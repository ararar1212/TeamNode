...
- `riskScore.js` — the heat-stress/fatigue scoring logic (pulse, ambient heat
  index, motion → green/yellow/red + a mitigation suggestion).
...
glowing markers with floating labels (pulse, gas, ambient, motion),
...
### ESP-NOW architecture (2 ESP32 boards)

Use two ESP32 boards:

- ESP32 sender (on helmet): reads all sensors and sends packets over ESP-NOW.
- ESP32 receiver (at laptop): receives ESP-NOW packets and prints one JSON line
  per packet over USB serial.
- Node server: reads serial JSON from the receiver and feeds the same
  `/api/readings` pipeline used by simulator/API clients.

From the PlatformIO project root:
...
For the laptop-side receiver board:
...
Then run the portal with the receiver serial port:

```bash
ESP32_SERIAL_PORT=/dev/ttyUSB0 ESP32_SERIAL_BAUD=115200 node server.js
```

If you want to run without hardware, disable serial ingestion:

```bash
ESP32_SERIAL_DISABLED=1 node server.js
```

### If you send directly over HTTP from ESP32
...
score is a **derived heat-stress/fatigue indicator** combining pulse,
ambient heat index, and motion — conceptually similar to
