  ...
  Object.values(workers).forEach(w => {
    ...
    card.innerHTML = `
      ...
      ${latest ? `
        ...
        <div class="metric-row"><span>Skin temp</span><b>${latest.skin_temp}°C</b></div>
...
function renderModalReadings() {
  ...
  modalReadings.innerHTML = latest ? `
    ...
    <div>Skin temp<b>${latest.skin_temp}°C</b></div>
...
function drawChart(history) {
  ...
  const data = {
    labels,
    datasets: [
      { label: "Pulse (bpm)", data: history.map(h => h.pulse), borderColor: "#eb5757", tension: 0.3 },
      { label: "Skin temp (°C)", data: history.map(h => h.skin_temp), borderColor: "#f2994a", tension: 0.3 },
      { label: "Risk score", data: history.map(h => h.risk_score), borderColor: "#3ddc97", tension: 0.3 },
    ],
  };
...
socket.on("reading", (record) => {
  ...
  if (activeWorkerId === record.worker_id) {
    ...
    fetch(`/api/workers/${activeWorkerId}/history?limit=40`)
      .then(r => r.json())
      .then(history => { drawChart(history); renderHistoryTable(history); });
  }
});
